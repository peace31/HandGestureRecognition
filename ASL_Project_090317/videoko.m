...
if isempty(face_bbox)
    ...
    s = regionprops(BWfinal,BWfinal2, 'Centroid','Area','BoundingBox','MeanIntensity');
    bw_area=[s.Area];
%     hist(bw_area);
    ...
    detectedImg=s(bw_region( winner)).BoundingBox;
    
    
     detectedImg=insertObjectAnnotation( BWfinal2,'rectangle',detectedImg,'Hand');
%                 x1=ceil(bbox(1));
%                 x2=round(x1+bbox(3));
%                 y1=ceil(bbox(2));
%                 y2=round(y1+bbox(4));
