...
%%%%Training Panel%%%%%
traininghand_panel= uipanel('Parent',main_fig,'Title','',...
    'Position',[0.5219+.0196+.225 0.0195 0.225 0.9723],'FontSize',8,'BorderType','none');
handtrainingbutton_control = uicontrol('Parent',traininghand_panel,'Style','pushbutton','String','Train Hand','Units','Normalize',...
    'Position',[0.05 0.85 0.9 0.125],'HorizontalAlignment','center','FontSize',8,'Callback',@trainhand_button);
...
set(adap_name_control,'String', Adaptor.ActiveAdaptor);%Input in Pop-up Menu Listof Active Adaptor
    ...
    function acqstart_button(hObject,eventData)
        ...
        load('emd.mat','alpha','weighted_feature');
        vid_adaptor_value=get(adap_name_control,'Value');
        vid_adaptor_string=get(adap_name_control,'String');
        vid_adaptor_selected=char(vid_adaptor_string(vid_adaptor_value));
        ...
        while keeplooping;
            frame = step(vid);
%             a_frame=getsnapshot(frame);
            detectedImg=handdetection_090317(frame); 

            if isempty(detectedImg)
                
            figure(1),imshow(frame,'Parent',ah2,'InitialMagnification','fit')
            else
                detectedImg1=insertObjectAnnotation( frame,'rectangle', detectedImg,'Hand');
            figure(1),imshow(detectedImg1,'Parent',ah2,'InitialMagnification','fit')
            end
...
%%%%%%%%%%%%%%%%%%%Train Hand%%%%%%%%%%%%%%%
function trainhand_button(hObject,eventData)
...
HandData_panel= uipanel('Parent',trainhand_fig,'Title','',...
    'Position',[0.0082 0.0195 0.225*1.5 0.95],'FontSize',8,'BorderType','none');
