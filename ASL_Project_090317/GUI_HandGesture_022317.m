...
%             a1=rgb2gray(frame);
%           
%            %Detect Edge using Range Filter 
%             e=rangefilt(a1);
%             ee=entropy(uint8(a1));eee=ee/10;
%             em=mat2gray(e);
% %             eee=graythresh(em);
%             
%             s1=im2bw(em,eee);
%              BWfilter=medfilt2(s1,[3 3]);
%             %Dilate the Image
%             se90 = strel('line', 9, 90);
%             se0 = strel('line', 9, 0);
%             BWsdil = imdilate(s1, [se90 se0]);
%             
%            %Fill Interior Gaps
...
%             ROI(BWfinal1~=1)=0;
%             SEG=BWfinal2.*ROI;
            figure(1),imshow(O,'Parent',ah2,'InitialMagnification','fit')
%            hold on;
            pos=[30,30,200,200];
